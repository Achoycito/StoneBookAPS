...
<head>
  ...
  <title>Editar etiqueta - StoneBook</title>
  ...
  <main id="main" class="main">
    ...
    <div class="card" style="background-color: #fff;">
        <div class="card-body">
            ...
            <?php
                $datosEtiqActual = $instCon->getDatosEtiqueta( $idEtiqActual );
                $etiqActual = $datosEtiqActual->fetch_assoc();
            ?>
            
            
            <form class="row g-3" action="etiquetas_be_actualizar.php?idEtiq=<?php echo $idEtiqActual; ?>" method="post">
                <div class="col-md-6">
                    <label class="form-label">Nombre de etiqueta</label>
                    <input required name="nombreEtiq" value="<?php echo $etiqActual["e_nombre"] ?>" type="text" maxlength="50" autocomplete="off" class="form-control" style="border-color: #05a649">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Color</label>
                    <select id="cmbColores" name="color" onchange="cambiarColor(this)" class="form-select">
                        <?php while($row = $colores->fetch_assoc()){?>
                          <option value="<?php echo $row["idColor"];?>" <?php if($etiqActual["idColor"] == $row["idColor"]){echo "selected"; $hexActual=$row["c_hex"];} ?> data-hex="#<?php echo $row["c_hex"];?>"><?php echo $row["c_nombre"];?></option>
                        <?php } ?>
                    </select>
                </div>
                ...
                <div class="col-md-12">
                  <div id="ejemplo" class="card col-md-12" style="background-color: #<?php echo $hexActual;?>; border: 2px solid black;">
                    <div class="card-body">
